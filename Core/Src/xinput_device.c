...
bool tud_xinput_n_ready(uint8_t itf) {
    ...
    return tud_ready() && (ep_in != 0) && !usbd_edpt_busy(BOARD_TUD_RHPORT, ep_in);
}
...
bool tud_xusb_n_report(uint8_t itf, void const *report, uint8_t len) {
    ...
    sending = true;
    return usbd_edpt_xfer(BOARD_TUD_RHPORT, p_xinput->ep_in, p_xinput->epin_buf, len);
}
...
bool tud_xinput_n_report(uint8_t itf, uint8_t report_id, void const *report,
    ...
    sending = true;
    return usbd_edpt_xfer(BOARD_TUD_RHPORT, p_xinput->ep_in, p_xinput->epin_buf,
                          len);
}
...
void xinputd_init(void) {
    xinputd_reset(BOARD_TUD_RHPORT);
}
...
uint16_t xinputd_open(uint8_t rhport, tusb_desc_interface_t const *itf_desc,
    ...
    if (TUSB_CLASS_VENDOR_SPECIFIC == itf_desc->bInterfaceClass) {
        ...
        if (itf_desc->bInterfaceSubClass == 0x01 && itf_desc->bInterfaceProtocol == 0xFF) {
            ...
            /* Tinyusb cannot open all endpoints in the configuration descriptor due
			 * to hardware limitations of the stm32f401cu6. In any case, we do not use
			 * the endpoints related to the headset interface.
             */
            if(itf_desc->bInterfaceProtocol != 0x03){
				TU_ASSERT(usbd_open_edpt_pair(rhport, p_desc, itf_desc->bNumEndpoints,
											  TUSB_XFER_INTERRUPT, &p_xinput->ep_out,
											  &p_xinput->ep_in),
						  0);
